...
const memberAuthController = {
  ...
  async handleSignupForm(req, res) {
    ...
    if (
      forbiddenWords.test(firstname) 
      || forbiddenWords.test(lastname)
      || forbiddenWords.test(email) 
      || forbiddenWords.test(password) 
      || forbiddenWords.test(password)
    ) {
      return res.render('signup', { 
        cssFile: 'signup.css', 
        pageTitle: 'signup', 
        errorMessage: 'Sérieusement?!' 
      });
    }
    ...
    if (!firstname || !lastname || !email || !password || !confirmation) {
      return res.render('signup', { 
        cssFile: 'signup.css', 
        pageTitle: 'signup', 
        errorMessage: 'Tous les messages sont obligatoires' 
      });
    }
    ...
    try{
      ...
      if (alreadyExistingUser) {
        return res.render('signup', { 
          cssFile: 'signup.css', 
          pageTitle: 'signup', 
          errorMessage: 'Membre déjà existant' 
        });
      }
  ...
  async handleLoginForm(req, res) {
    ...
    const forbiddenWords = /\b(insert|drop|update|select|delete|create|alter)\b/i;

    if (forbiddenWords.test(email) || forbiddenWords.test(password)) {
      return res.render('signup', { 
        cssFile: 'signup.css', 
        pageTitle: 'signup', 
        errorMessage: 'Sérieusement?!' 
    ...
    try {
      ...
      if (!member) {
        return res.render('login', { 
          cssFile: 'login.css', 
          pageTitle: 'Login', 
          errorMessage: 'Mauvais couple email/password' 
        });
      }
